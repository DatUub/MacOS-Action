...
const escapeRegExp_js_1 = tslib_1.__importDefault(require("lodash/escapeRegExp.js"));
...
class Client extends discord_js_1.Client {
    ...
    async initGuildApplicationCommands(guildId, DCommands, options) {
        ...
        const guild = this.guilds.cache.get(guildId);
        if (!guild) {
            this.logger.warn(`${this.user?.username ?? this.botId} >> initGuildApplicationCommands: skipped (Reason: guild ${guildId} unavailable)`);
            return;
        }
    ...
    executeCommandInteraction(interaction) {
        ...
        if (!applicationCommand?.isBotAllowed(this.botId)) {
            if (!this.silent) {
                this.logger.warn(`${this.user?.username ?? this.botId} >> interaction not found, commandName: ${interaction.commandName}`);
            }
            return;
        }
    ...
    async executeComponent(components, interaction) {
        const executes = components.filter((component) => component.isId(interaction.customId) &&
            component?.isBotAllowed(this.botId));
        if (!executes.length) {
            if (!this.silent) {
                this.logger.warn(`${this.user?.username ?? this.botId} >> ${interaction.isButton()
                    ? "button"
    ...
    executeContextMenu(interaction) {
        ...
        if (!applicationCommand?.isBotAllowed(this.botId)) {
            if (!this.silent) {
                this.logger.warn(`${this.user?.username ?? this.botId} >> context interaction not found, name: ${interaction.commandName}`);
            }
            return;
        }
        return applicationCommand.execute(this.guards, interaction, this);
    ...
    async getMessagePrefix(message) {
        if (typeof this.prefix !== "function") {
            return (0, isArray_js_1.default)(this.prefix) ? [...this.prefix] : [this.prefix];
        }
    ...
    async parseCommand(prefix, message, caseSensitive = false) {
        ...
        const prefixRegex = RegExp(`^(${[...prefix, ...mappedPrefix]
            .map((pfx) => (0, escapeRegExp_js_1.default)(pfx))
            .join("|")})`);
